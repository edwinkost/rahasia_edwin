...
class GroundwaterModflow(object):
    ...
    def set_bcf_for_two_layer_model(self):
        ...
        if self.usePreDefinedConfiningLayer:
            # - reduce horizonal conductivity for the confining layer part
            horizontal_conductivity_layer_2 = pcr.max(minimimumTransmissivity, \
    ...
    def set_well_package(self, gwAbstraction):
        ...
        abstraction = gwAbstraction * self.cellAreaMap * pcr.scalar(-1.0) * 100000. * self.cellAreaMap/(pcr.clone().cellSize()*pcr.clone().cellSize())
